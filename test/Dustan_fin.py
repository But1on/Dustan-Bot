...
bot = telebot.TeleBot('6401641976:AAGJ79i5DjEyqzZyVZ-wL5qwsmyVNPr7E4E') #айд бота

nickname = '' #global marks
password = ''
energy = 0
time_out = 1
build = ''
...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Приветствую вас, я бот-помощник, и с моей помощью вы сможете создавать и публиковать сборки для игры Destiny 2, ...', reply_markup=markup)
    bot.register_next_step_handler(message,on_click)

def on_click(message):
    global users
    conn = sqlite3.connect('users.sql')#название таблицы
    cur = conn.cursor()
    
    cur.execute('CREATE TABLE IF NOT EXISTS users(id int auto_increment primary key, name vachar(50),pass varchar(50), rating_count INTEGER, count_voice INTEGER, all_class TEXT, all_element TEXT)')#условие создание,если ее еще нет
    conn.commit()
...
''' sign in '''  
def signiner(message):#функция входа
    global users
    global nickname
    o = 0
    for user in users:
...
''' registration '''
def registering(message):#функция регистрации
    global nickname
    global users
...
''' main '''
def main(message): #основное тело программы
    global users
    global password
    global nickname
    global buildes
    ...
    conn = sqlite3.connect('buildes.sql') #название таблицы
    cur = conn.cursor()
        
    cur.execute('CREATE TABLE IF NOT EXISTS buildes(id INTEGER PRIMARY KEY, name TEXT, name_build TEXT,rating INTEGER, raiting_count INTEGER, value TEXT)') #условие создание,если ее еще нет
    ...
    conn = sqlite3.connect('mark.sql')#название таблицы
    cur = conn.cursor()
        
    cur.execute('CREATE TABLE IF NOT EXISTS mark(id INTEGER PRIMARY KEY, name TEXT, name_build TEXT, name_voice TEXT, mark INTEGER)') #условие создание,если ее еще нет
    conn.commit()
    cur.close()
    conn.close()
...
''' создатель '''
def choose_class(message):#создание сборки
    global build
    global all_info_game
    global time_out
    global markup_speshal
